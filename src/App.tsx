...
export default function App() {
  ...
  const [profileInputs, setProfileInputs] = useState({ income: "", cash: "", investments: "" });

  const fmtInput = (raw: string) => {
    const n = parseInt(raw.replace(/\D/g, ""), 10);
    return isNaN(n) ? "" : new Intl.NumberFormat("vi-VN").format(n);
  };
  const parseInput = (raw: string) => parseInt(raw.replace(/\D/g, ""), 10) || 0;

  const openProfileEdit = () => {
    setProfileInputs({
      income: profile.monthly_income.toString(),
      cash: profile.cash_balance.toString(),
      investments: profile.investments_balance.toString(),
    });
    setEditingProfile(true);
  };

  const saveProfileEdit = () => {
    const income = parseInput(profileInputs.income);
    const cash = parseInput(profileInputs.cash);
    const investments = parseInput(profileInputs.investments);
    if (income > 0) {
      handleProfileSave({ ...profile, monthly_income: income, cash_balance: cash, investments_balance: investments });
    }
    ...
    <div className="w-full min-h-screen bg-[#F0F2F5] flex flex-col font-sans overflow-x-hidden text-black relative select-none">
      ...
      <main className="flex-1 flex flex-col lg:flex-row gap-6 p-4 md:p-6 pb-20 max-w-7xl w-full mx-auto">
        ...
        <div className="w-full lg:w-80 shrink-0 flex flex-col gap-6">
          {/* Financial profile quick-edit card */}
          <div className="bg-white border-4 border-black rounded-3xl p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-black">
            <div className="flex items-center justify-between border-b-2 border-black pb-2 mb-4">
              <h4 className="font-black text-xs uppercase">💰 Hồ sơ tài chính</h4>
              {editingProfile ? (
                <div className="flex gap-1.5">
                  <button onClick={() => setEditingProfile(false)} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 border-2 border-black rounded-lg text-[10px] font-black cursor-pointer">Huỷ</button>
                  <button onClick={saveProfileEdit} className="px-2.5 py-1 bg-emerald-500 hover:bg-emerald-600 text-white border-2 border-black rounded-lg text-[10px] font-black cursor-pointer flex items-center gap-1">
                    <Check className="w-3 h-3" /> Lưu
                  </button>
                </div>
              ) : (
                <button onClick={openProfileEdit} className="flex items-center gap-1 px-2.5 py-1.5 bg-slate-100 hover:bg-yellow-200 border-2 border-black rounded-lg text-[10px] font-black uppercase cursor-pointer transition-all">
            ...
            {[
              { label: "Thu nhập / tháng", key: "income" as const, value: profile.monthly_income, color: "text-emerald-700", required: true },
              { label: "Tiền mặt / TK ngân hàng", key: "cash" as const, value: profile.cash_balance, color: "text-sky-700", required: false },
              { label: "Đầu tư (CK, quỹ, crypto)", key: "investments" as const, value: profile.investments_balance, color: "text-violet-700", required: false },
            ].map(({ label, key, value, color, required }) => (
              <div key={key} className="mb-3 last:mb-0">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">{label}{required ? " *" : ""}</span>
                {editingProfile ? (
                  <div className="relative">
                    <input
                      type="text"
                      autoFocus={key === "income"}
                      value={fmtInput(profileInputs[key])}
                      onChange={(e) => setProfileInputs((p) => ({ ...p, [key]: e.target.value }))}
                      onKeyDown={(e) => { if (e.key === "Enter") saveProfileEdit(); if (e.key === "Escape") setEditingProfile(false); }}
                      className="w-full px-3 py-2 bg-slate-50 border-2 border-indigo-400 rounded-xl font-mono font-black text-sm outline-none pr-6"
                    />
                    <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs">₫</span>
                  </div>
                ) : (
                  <span className={`font-mono font-black text-base ${color}`}>
            ...
            {!editingProfile && profile.cash_balance === 0 && profile.investments_balance === 0 && (
              <p className="mt-3 text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-300 rounded-xl px-3 py-2">
                ⚠️ Chưa có số dư → Hội đồng dễ từ chối. Bấm Sửa để điền vào.
              </p>
            )}
          </div>
